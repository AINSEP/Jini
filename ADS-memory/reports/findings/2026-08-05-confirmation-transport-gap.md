...
- **Jini's own build-time guard**, `packages/cms/src/core/tools/registration-kit.ts:113-130`:
  ...
  **Verified directly:** `collections_execute_cleanup` is explicitly in `packages/cms/src/content-types/tool-registrations.ts:77`'s `UNWIRED_CONTENT_TYPES_TOOL_IDS` set, so the guard's throw is never actually exercised for it (it's kept out of the registered set deliberately, not caught by the throw at runtime). **Not independently verified by me**, only cited consistently across two source comments (`registration-kit.ts:123-124` and `packages/cms/src/taxonomy/agent-tools.ts:30-36`): that `database_execute_migrate_forward` and `backup_execute_restore` carry the same rule and are also deliberately left unwired. I did not individually chase each of those two tools' own catalog entry and unwired-list membership file by file — flagging that as inferred-from-citation, not independently read the way `collections_execute_cleanup`'s was.
...
**Read directly, this session:** `tool-executor.ts:213, 238-247, 350-361, 408-442, 507-515`; `agent-daemon-server.ts:354` (Tovu); `registration-kit.ts:113-130, 378-381`; `pending-confirmations.ts:16-21`; `frontend-capability-tools.ts:127-163`; `frontend-control.ts` (full file, this session's earlier trace); `chat-capabilities.ts` (full file); `page-executor.ts`'s module doc; `frontend-session-bridge.ts:1-40, 81-88`; `content-types/tool-registrations.ts:77`'s `UNWIRED_CONTENT_TYPES_TOOL_IDS`; grep confirming zero `resumeConfirmation` callers in Tovu's `src/`.

**Cited from source comments, not independently re-derived by chasing each tool's own catalog file:** that `database_execute_migrate_forward` and `backup_execute_restore` specifically carry `confirmer-must-equal-own-delegatedBy` and are declared unwired (asserted consistently in two separate comments, `registration-kit.ts:123-124` and `taxonomy/agent-tools.ts:30-36`, but not verified against each tool's own catalog entry the way `collections_execute_cleanup`'s was).
