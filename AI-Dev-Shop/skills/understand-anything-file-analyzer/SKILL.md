...
**Capture load-bearing invariants when the file states one.** If the source or its module doc
asserts that this is the *only* way to do something, the *sole* path through a boundary, the
canonical entry point, or that some rule is enforced here and nowhere else, put that in the
summary verbatim-in-spirit. These sentences are what stop a future reader from building a
second, weaker path alongside an existing one — they are worth more than a restatement of the
file's exports.

Good: "The sole tool-invocation boundary: enforces policy gating, confirmation, timeout, and
audit. Handlers are unreachable except through this module."
Good: "Canonical run-start path — every transport (HTTP, CLI, MCP) funnels through here."

Do not invent invariants. Only record one when the code or its doc comment actually claims it.
...
Skip trivial one-liners, simple re-exports, and auto-generated boilerplate.

**Exported interfaces and contract types are `class:` nodes — do not skip them.** In
interface-driven codebases (ports-and-adapters, hexagonal, dependency-injection, or any
project where `SomeThing` is an interface and `createSomeThing`/`SomeThingImpl` is one of
several implementations), the interface *is* the architectural unit. A graph containing only
the concrete implementations cannot answer "what already implements this contract?" — the
single most valuable question an agent asks before writing a new implementation.

Emit a `class:<path>:<Name>` node for every exported `interface`, `abstract class`, `trait`,
`protocol`, or contract-shaped `type` that is:
- implemented, extended, or satisfied by something else in the project, OR
- consumed as an injected/constructor/parameter dependency by another file, OR
- named in a DI token, registry, or factory signature.

Still skip genuinely trivial aliases (`type Id = string`), single-use inline option bags, and
prop-type interfaces local to one component.

**Target the contract, not an implementation.** When a consumer depends on an interface, the
edge target is the interface node — never a sibling concrete implementation. Emitting
`consumer --depends_on--> createInMemoryFooStore` when the consumer actually accepts an
injected `FooStore` is a factual error: it invents a coupling the code deliberately avoids,
and it hides the fact that other implementations are substitutable. If you catch yourself
writing an edge label that qualifies or walks back the edge itself (e.g. "consumes the port,
not necessarily this impl"), the edge is wrong — repoint it at the interface node.
...
- **A wrong edge is worse than a missing edge.** A missing edge makes a reader look harder; a
  false edge makes them confidently wrong, and nothing downstream can catch it — the merge
  script only drops edges whose *nodes* are missing, so a fabricated edge between two real
  nodes survives into the final graph unchallenged. Every edge you emit must be grounded in
  one of: `batchImportData` (for `imports`), the script's `callGraph` (for `calls`), an
  `extends`/`implements` clause the script extracted, or a symbol you can point to in
  `neighborMap`. If you cannot name the specific evidence, do not emit the edge.
- **Never repurpose an edge to avoid a dangling reference.** If a file depends on something
  outside your batch and outside `neighborMap` (a cross-package import, an external module,
  an unresolved symbol), the correct action is to OMIT the edge — not to retarget it onto a
  same-batch file so it resolves. Retargeting produces a dependency that does not exist. A
  tell-tale symptom is an edge whose label describes a different target than the one in
  `target`; if you are writing such a label, delete the edge instead.
...
- **Emit exactly the schema's edge fields: `source`, `target`, `type`, `direction`, `weight`.**
  `direction` is REQUIRED on every edge and is always `"forward"` for this agent — omitting it
  is a schema violation. Do NOT add fields the schema does not define (`label`, `note`,
  `reason`, `confidence`, etc.); they are silently discarded, so any meaning you put there is
  lost. If a relationship needs prose to be understood, that prose belongs in the source or
  target node's `summary`, not on the edge.
