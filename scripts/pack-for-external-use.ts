 ...
 *
 * Packs real `@jini-ai/*` tarballs for use in a project OUTSIDE this repo, without any real npm
 ...
 *
 * Usage:
 *   tsx scripts/pack-for-external-use.ts                    # every @jini-ai/* package
 *   tsx scripts/pack-for-external-use.ts --only core,daemon # just these + their @jini-ai/* deps
 *
...
async function main(): Promise<void> {
  ...
  console.log(
    '\nTo use these in another project, add to its package.json "dependencies" (only the ones you actually need — file: entries pull in their own @jini-ai/* deps automatically via the rewritten tarballs):\n',
  );
  console.log(`  {\n${usageLines.join(',\n')}\n  }`);
