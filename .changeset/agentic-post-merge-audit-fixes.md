---
"@jini-ai/agentic": patch
---

Close seven correctness/safety findings in `@jini-ai/agentic`, each reproduced by a failing test
...
and handed the guard only whichever resolved *first*. So the page, not the guard, chose which of
...
`contenteditable` region's text — which *is* the field's value, not page chrome — was reported
...
clean and replaced that surface's *entire data model* with `undefined` — silently, with no error
...
repetition along one path but never bounded the tree's *size*: 24 components each naming the next
