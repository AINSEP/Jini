 ...
 * investigation, which found `@injini/node-host`'s `createLocalNodeDaemon` wrote nothing anywhere a
 * separate CLI process could read. That daemon side is now built (`@injini/node-host`'s
 * `discoveryFile` config, backed by `@injini/sidecar`'s `daemon-registry.ts`); this module is the
 ...
 * product's daemon keeps its data — the caller (whatever wires a `@injini/node-host` daemon and a
 * `@injini/cli`-transport pack together for one product) supplies the same `dataDir` (or an
...
export interface LocalDaemonDiscoveryOptions {
   ...
   * registry path is derived from it via `@injini/sidecar`'s `resolveDaemonRegistryPath` — the
   * identical derivation `@injini/node-host` uses for its own `discoveryFile` default. Ignored when
