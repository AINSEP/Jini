 ...
 * `Sandbox.create()` call and the `wrapE2bSandbox` config — both the SDK and the wrapping logic
 * are mocked out so what's under test is that translation, not a real sandbox boot. The
 * assertion that matters is the *shape* of what gets passed through: default-filling, and that
 * an omitted config key is genuinely absent from the SDK call rather than present as `undefined`
 * (this package's `exactOptionalPropertyTypes: true` makes that distinction real, not cosmetic).
 */
...
const FAKE_SANDBOX = { fake: 'sandbox-handle' };
...
describe('createE2bSandboxProvider', () => {
  it('applies its own defaults when config and boot options are both empty', async () => {
    ...
    expect(wrapE2bSandboxMock).toHaveBeenCalledWith(FAKE_SANDBOX, {
