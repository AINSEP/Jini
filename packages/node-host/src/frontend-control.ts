 ...
 * addresses a run to a surface, `@jini-ai/http`'s frontend-session routes carry invocations to a
...
function defaultBindErrorSink(context: FrontendBindErrorContext): void {
  ...
  console.error(`[@jini-ai/node-host] could not bind run "${context.runId}" to a frontend surface`, context.error);
}
