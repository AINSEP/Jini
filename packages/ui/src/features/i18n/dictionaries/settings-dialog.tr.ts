...
export const SETTINGS_DIALOG_TR: SettingsDialogDict = {
  ...
  'This is shorter than any provider API key — check that the whole key was pasted. You can still save and test it.': 'Bu, herhangi bir sağlayıcının API anahtarından daha kısa — anahtarın tamamının yapıştırıldığından emin olun. Yine de kaydedip test edebilirsiniz.',
  'This is what a completion notification looks like.': 'Tamamlanma bildirimi böyle görünür.',
  'This looks like an API key for {vendor}, not {provider}. You can still save and test it.': 'Bu, {provider} için değil {vendor} için bir API anahtarına benziyor. Yine de kaydedip test edebilirsiniz.',
