...
export const SETTINGS_DIALOG_ZH_TW: SettingsDialogDict = {
  ...
  'Testing…': '正在測試…',
  'This is shorter than any provider API key — check that the whole key was pasted. You can still save and test it.': '這比任何供應商的 API 金鑰都短——請檢查是否貼上了完整的金鑰。你仍然可以儲存並測試它。',
  'This is what a completion notification looks like.': '完成通知看起來就像這樣。',
  'This looks like an API key for {vendor}, not {provider}. You can still save and test it.': '這看起來像是 {vendor} 的 API 金鑰,而不是 {provider} 的。你仍然可以儲存並測試它。',
