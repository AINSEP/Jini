...
export const SETTINGS_DIALOG_UK: SettingsDialogDict = {
  ...
  'Testing…': 'Перевірка…',
  'This is shorter than any provider API key — check that the whole key was pasted. You can still save and test it.': 'Це коротше, ніж будь-який ключ API постачальника — перевірте, чи вставлено весь ключ повністю. Ви все одно можете зберегти й перевірити його.',
  'This is what a completion notification looks like.': 'Ось як виглядає сповіщення про завершення.',
  'This looks like an API key for {vendor}, not {provider}. You can still save and test it.': 'Схоже, це ключ API для {vendor}, а не для {provider}. Ви все одно можете зберегти й перевірити його.',
