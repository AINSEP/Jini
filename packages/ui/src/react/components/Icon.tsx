...
// The fully-resolved props every icon renders with: size, stroke, and any
// caller overrides already merged in (see `Icon` below). A lookup table
// keyed by every `IconName` — TypeScript enforces the key set is exhaustive,
// so a name added to the union without a matching entry here is a compile
// error rather than a silent `default: return null`.
type CommonIconProps = SVGProps<SVGSVGElement>;
type IconRenderer = (common: CommonIconProps) => ReactElement;

// Exported as a test seam only — call it directly to assert key-set
// completeness and per-entry validity without rendering. NOT part of the
...
export const ICON_RENDERERS: Record<IconName, IconRenderer> = {
    ...
    'spinner': (common) => (
        <svg {...common} className={`icon-spin ${common.className ?? ''}`.trim()}>
