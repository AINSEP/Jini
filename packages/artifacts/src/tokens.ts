/**
 * `@injini/core` DI token for `ArtifactStore`.
 *
 * Moved here from `@injini/daemon/src/tokens.ts` on 2026-07-19: the locked kernel-noun set
 ...
 * `ToolExecutorToken`, `AgentExecutorToken`) in `@injini/daemon` — a real, shipped violation of
 ...
 * debate flagged. `@injini/artifacts` is an unlocked (incubating) package — see `UNLOCKED.md` —
