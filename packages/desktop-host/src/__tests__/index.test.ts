...
describe('@jini-ai/desktop-host root barrel', () => {
  ...
  it('does NOT export the Electron or Tauri shell assemblies — those live at ./electron and ./tauri', () => {
    // Split 2026-07-29: a real app only ever ships one shell, so the root entry point must never
    // statically pull in both. Confirmed absent here rather than assumed.
    const shellOnlyNames = [
    ...
    for (const name of shellOnlyNames) {
      expect((barrel as Record<string, unknown>)[name], `"${name}" should not be on the root barrel`).toBeUndefined();
    }
  });
});
