/**
 * @injini/chat-core's internal `agentic/` barrel — the chat pane's own capability manifest. Not a
 ...
 * markup convention, and the protocol projections) moved to `@injini/agentic` on 2026-07-26 — see
 ...
 * than generic vocabulary, and re-exports `CapabilityDef` from `@injini/agentic` purely for
 * `chat-react`'s older `agent-tools.ts` compatibility shim (which still names it via
 * `@injini/chat-core`) — new code should import `CapabilityDef` from `@injini/agentic` directly.
 */
export { CHAT_CAPABILITIES } from './chat-capabilities.js';
export type { CapabilityDef, CapabilityInputSchema, CapabilityRisk } from '@injini/agentic';
