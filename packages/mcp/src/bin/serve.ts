 ...
 *
 * Follows `@injini/cli/main.ts`'s established shape for a bootable, side-effect-guarded, `#!/usr/bin/env
 ...
 * calling side effect — bundling it into the library barrel would mean `import '@injini/mcp'` could
 ...
 *
 * **Daemon-URL resolution** mirrors `@injini/cli/main.ts`'s own posture: no baked-in default, just
 * `resolveDaemonUrl` (`@injini/cli`) wired to a `--daemon-url`-shaped precedence chain — except this
...
import { resolveDaemonUrl as resolveDaemonUrlDefault, type ResolveDaemonUrlOptions } from '@injini/cli';
...
/** Checked (only) when resolving the daemon HTTP base URL — see `resolveDaemonUrl`'s own docs; matches `@injini/cli/main.ts`'s identically-named constant. */
...
export interface ServeDeps {
  ...
  /** @default the real @injini/cli resolveDaemonUrl */
...
// tests importing the named `serve` export above. Mirrors `@injini/cli/main.ts`'s identical guard.
