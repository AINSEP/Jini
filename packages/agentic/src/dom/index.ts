/**
 * @injini/agentic/dom — the browser half of agent control.
 *
 * The one `PageDriver` (see `@injini/agentic`'s `page-driver.ts`) that reads and writes a real DOM
 ...
 *
 * `@injini/chat-react` is this package's one in-repo consumer today.
 */
export { createDomPageDriver, currentAgentPage, type DomPageDriverOptions } from './dom-page-driver.js';

/**
 * WebMCP feature detection — moved 2026-07-26 from `@injini/ui`'s `agent-tools/model-context.ts`
