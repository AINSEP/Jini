...
describe('page.find_elements', () => {
  it('returns every published element with pages and an untrusted-content label', async () => {
    ...
    expect(result.untrustedFields).toContain('pages[].label');
  });
...
describe('page.navigate', () => {
  ...
  it('bounds and strips the pages it advertises as available — both id and label, not just the caller\'s argument', async () => {
    // `pages[].id` and `pages[].label` both come straight from the host's own `data-agent-page`
    // markup — page-authored text with no length bound, the same shape of risk as the caller's
    // `page` argument, in the exact same thrown message. Both fields carry a hostile payload here
    // so a fix that bounds only one of them still fails this test.
    const hostilePages = createFakeDriver({
      listPages: vi.fn(async () => [
        { id: `‮long-page-name-${'y'.repeat(500)}`, label: `‮long-page-label-${'z'.repeat(500)}` },
      ]),
    });
    let caught: Error | undefined;
    try {
    ...
    expect(caught).toBeInstanceOf(Error);
    // Two hostile fields bounded independently (id and label each capped at MAX_AGENT_LABEL_LENGTH,
    // 200), not one combined field — the message is bounded, just not as tightly as the
    // single-hostile-field case above, which is why this threshold is looser than that one's.
