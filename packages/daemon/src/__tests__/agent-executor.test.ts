...
describe('AgentExecutor — runtimeLock (antigravity model-selection mutex)', () => {
  ...
  it('releases the lock when buildArgs itself throws — the settings.json write it guards can fail', async () => {
    // The lock is acquired BEFORE buildArgs precisely so buildArgs' process-global
    // settings.json write is serialized. That write is real filesystem IO
    // (mkdirSync/writeFileSync) and can throw on a read-only home, a permissions
    // failure, or a full disk. With no try/finally around the buildArgs call the
    // hold is never released and no process ever exists to fire the exit-based
    // release, so every later concrete-model run on this def blocks forever.
    const recording = createRecordingLock();
    const stager = createFakeLogFileStager();
    const def = createLockedDef(recording.lock, {
      buildArgs: () => {
        throw new Error("EACCES: permission denied, mkdir '/root/.gemini'");
      },
    });
    const { lifecycle, executor } = createHarness({ def, prepareAgentLogFile: stager.prepareAgentLogFile });
    const { run } = await lifecycle.start({ contextRef: 'ctx-1' });
    ...
    // Released, handoff never started (no process could have consumed the write),
    // and the staged log file cleaned up like every other pre-spawn failure path.
    expect(recording.events).toEqual(['acquire', 'release']);
    expect(stager.cleanup).toHaveBeenCalledTimes(1);
  });
