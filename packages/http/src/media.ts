 ...
 * A product composition root may satisfy those ports with `@injini/media`; locked `@injini/http`
 * deliberately does not point down into that incubating implementation package.
 *
 * Follows the "request-now, poll-later" async-task shape `@injini/media`'s own `MediaTaskStore` doc
 ...
 *
 * **Where generated bytes land** (per `@injini/media`'s own boundary doc on `MediaGenerationRequest`/
