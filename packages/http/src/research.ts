 ...
 * instead; `@jini-ai/http` deliberately does not depend on the incubating media implementation
...
function defaultInternalErrorSink(context: ResearchInternalErrorContext): void {
  ...
  console.error(`[@jini-ai/http] internal error (research/search, correlationId=${context.correlationId})`, context.error);
}
