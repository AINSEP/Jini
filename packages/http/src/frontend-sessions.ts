 ...
 *
 * The transport half of `@jini/daemon`'s `FrontendSessionRegistry`: how a browser surface reaches
 ...
 * directly — see `@jini/daemon`'s own module doc for why attach and bind are separate.
 */
import { randomUUID } from 'node:crypto';
import type { Express, Request, Response } from 'express';
import { createApiError } from '@jini/protocol';
import type { FrontendSessionHandle, FrontendSessionRegistry } from '@jini/daemon';
...
function reportInternalError(deps: FrontendSessionsHttpDeps, source: string, error: unknown): void {
  try {
    ...
    else console.error(`[@jini/http] internal error (frontend-sessions:${source})`, error);
  } catch (sinkError) {
    // A diagnostic sink must never turn a contained failure into an unhandled rejection of its own.
    // eslint-disable-next-line no-console
    console.error(`[@jini/http] internal error sink failed (frontend-sessions:${source})`, sinkError);
  }
}
