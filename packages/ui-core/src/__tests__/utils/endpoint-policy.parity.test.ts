...
/**
 * @file Holds `ui-core`'s `endpoint-policy.ts` and `agent-runtime`'s
 * `connection-guard.ts` in agreement mechanically.
 *
 * The copy between them is deliberate and documented (`endpoint-policy.ts`'s
 * header): `ui-core` ships zero dependencies so it can run in a browser bundle,
 * and `connection-guard`'s richer half needs `node:dns`. What was missing was
