...
// Mirror of `@injini/platform`'s internal `quoteWindowsCommandArg`, kept
...
// `@injini/platform`'s `createCommandInvocation` hands to `spawn` —
...
export function checkWindowsCmdShimCommandLineBudget(
  ...
  // `cmd.exe /d /s /c "<inner>"` — same shape `@injini/platform`'s
